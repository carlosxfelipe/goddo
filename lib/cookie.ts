...
export class Cookie {
  ...
  constructor(value: string | undefined, attrs: CookieAttributes = {}) {
    this.#value = value
    this.#attrs = { ...attrs }
  }
  ...
  constructor(cookieHeader: string | null) {
    if (cookieHeader) {
      for (const pair of cookieHeader.split(';')) {
        ...
        this.#cookies.set(key, new Cookie(val))
      }
    }
    ...
    return new Proxy(this, {
      ...
      set(target, prop: string, value: unknown): boolean {
        ...
        if (value instanceof Cookie) {
          ...
          const cookie = target.#cookies.get(prop) ?? new Cookie(undefined)
